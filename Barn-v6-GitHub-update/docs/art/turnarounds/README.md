...
本页记录的前一批按当时用户指示，先画牛棚、棚屋、马厩的正／左／右／后四个方向，暂缓透明度和 Reveal。用内置图像生成工具制作三套对照板；用户随后开始逐张审阅，先修改 Barn。包括最新侧面宽度反馈共完成 15 次生成／修订，当前三套共 12 个方向；留存 10 张板，含 7 张历史／编辑来源板。**这是待审美术，不是可加载的四向 sprite 套装。**

2026-09-16 接续：普通空牛棚 0.1.3 基本实机验证已收尾，本批回到 Barn 屋面修订，新增 v6。此前马厩侧面显露及棚屋室内对照的已完成示意见[使用研究](../use-studies/README.md)，不再列为从零重做的前置任务。

前一批左右标签反了。按用户指出的建筑左右侧命名，当前 Barn v6、Shed v3、Stable v4 均已统一为：左上 FRONT、右上 RIGHT、左下 LEFT、右下 BACK。面板位置没交换。旧图标签仍有错误，不能混用为当前方向。

美术侧视名称与游戏朝向枚举分开：当前面板的游戏朝向仍是左上 South、右上 West、左下 East、右下 North。纠正文字不修改核心旋转方向或建筑门位。

## 牛棚 Barn

![牛棚四视图 v6：侧向瓦片间距与坡面明暗修订](barn-four-views-v6.png)

2026-09-16 的 v6 以 [v5](barn-four-views-v5.png) 为来源，局部修订两张侧面屋顶：上部瓦片排距压缩，下部瓦片露出更多且颜色较深，叠瓦轮廓取代原先过于规则的砖墙感。已目视检查：左右标签、水平屋脊／屋檐、左侧干净的顶边、右侧附属小屋面上的扁平交叉木条均保留；正背面主体结构保持，但纹理仍有重绘。曲面体积感比 v5 更清楚，仍不把本次结果记为用户定稿。

v6 是 1448×1086 RGB 对照板。侧面深色轮廓宽为 RIGHT 267、LEFT 268 板上像素，不是 16 像素一格的游戏素材。下一步需按正式底稿重画源像素版本，保持已实测门位：South 人门地面门槛 (24,112)、West (0,72)、East (64,136)、North (88,48)。侧向门不能为了露出来而贴到当前可见红墙；本批没有完成入口视觉重构、alpha 分层或游戏贴图接入。完整提示词、图片散列及检查见 [本批记录](inspection-2026-09-16.json)。
...
此前 v5 以 [v4](barn-four-views-v4.png) 为来源，把右视图原先立在红墙前的大叉压入附属小棚屋面的浅窄可见范围，恢复其上方的红墙。已查看结果：交叉木条落在小屋面范围内，不再跨过红墙；已修正的左右标签和左侧屋顶轮廓保留。此版没有用户定稿确认。生成有细部纹理重绘，不宣称其他区域逐像素不变，也不把目视检查视为严格三维投影验收。

v5 的侧向瓦片排列过于整齐、屋顶像竖板，是 v6 本次处理的问题。v6 改善了排距与明暗，但曲面、跨视图比例和构件尺度仍待审。
...
马的可见性、停放和骑乘通路尚未实现，本图没有添加马，也没有改变通行数据。前一批按用户要求先收完美术进度；本轮室内开关／自动打包和马厩侧面显露已另作讨论记录。水草兼容仍未确认，侧面显露与真实骑乘均尚未实现。
...
当前待审为 Barn v6、Shed v3、Stable v4；本批仅新增 Barn 屋面修订，Shed 和 Stable 保留原稿。透明度、分层、原生分辨率与逐像素门位校准留到形体方向确定后；不再先追着一张 Reveal 图反复调整透明度。
