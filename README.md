...
**状态：已有独立几何与编辑逻辑，尚不可安装游玩。** 已新增运行流程接入层与尚未编译的 SMAPI 诊断入口源码，仍没有成品贴图或游戏内测试结果；不要把本仓库直接放进 `Mods` 当作已完成的 mod。
...
- `src/BuildingRotation.Smapi`：尚未编译的加载／诊断入口源码，不含游戏旋转补丁。
...
核心库和数据层使用 `netstandard2.1`，自检及离线检查工具用 `net8.0`。它们不引用游戏程序集，也不读写存档。SMAPI 诊断工程的实际编译和版本依赖仍待游戏环境核验。

2026-09-10：加入 Runtime 后五个无游戏依赖工程的标准 MSBuild 构建通过，**100/100 独立自检通过**（新增 12 组）。SMAPI 入口没有编译、没有实机测试；细节见[接入记录](docs/runtime-adapter.md)。
