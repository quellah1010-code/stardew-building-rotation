...
1. 按用户最新节奏逐张审图，当前待审图为 `barn-four-views-v5.png`、`stable-four-views-v4.png`、`shed-four-views-v3.png`，均在 `docs/art/turnarounds`；接着按用户反馈修订。修形体、屋面和比例，不自行跳回透明度调参。交叉木条必须随附属屋面投影，不能把正面大叉直接贴到侧面红墙上。四向形体确定后再做原生画稿、透明分层和入口视觉重构。正式地面坐标仍以 `docs/art/templates` 为依据，改门位必须同步布局与底稿，不能只改画面。用户不需要代画。
