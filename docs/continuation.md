...
最新为 Barn v5、Stable v3、Shed v2，共 3 张当前板。Stable 两侧外轮廓均约为正面的 52.1%（原 41.2%～43.2%）；Shed 约 44.5%～44.9%（原 36.3%）。本轮 5 次生成，淘汰 3 次 Stable 偏差稿，最终保留两张；四视图批次累计 13 次生成、7 张留档。三份当前图的左右标签均已统一。图片完整解码、轮廓宽度测量和目视检查完成，没有新的游戏内或 C# 测试。
...
1. 按用户最新节奏逐张审图，当前待审图为 `barn-four-views-v5.png`、`stable-four-views-v3.png`、`shed-four-views-v2.png`，均在 `docs/art/turnarounds`；接着按用户反馈修订。修形体、屋面和比例，不自行跳回透明度调参。交叉木条必须随附属屋面投影，不能把正面大叉直接贴到侧面红墙上。四向形体确定后再做原生画稿、透明分层和入口视觉重构。正式地面坐标仍以 `docs/art/templates` 为依据，改门位必须同步布局与底稿，不能只改画面。用户不需要代画。
