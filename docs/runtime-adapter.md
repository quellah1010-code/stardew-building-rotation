...
2026-09-15 最新 0.1.2 实机证据：日志确认普通 Barn 朝东／北／西提交及侧向 4×7、北向 7×4 的重选读取；用户确认放下后绕行没有穿墙或外部卡住，各方向均可进门并从对应一侧返回，附图显示普通牛棚室内。基本碰撞与人门流程通过，非法放置、取消、存读档等仍待验收。用户继续在旧档不过夜测试；预览重叠待修。见[实机记录](rotation-input-investigation.md)。
