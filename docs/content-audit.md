...
这次完成的是数据检查和接续资料；没有新增运行时适配代码，也没有声称真实数据已通过游戏内验证。63 项自检仍是上一次核心提交的结果。
...
先实现真实字段到核心布局的读取／映射，以 Barn、Shed 和 Stable 做独立数据验证；同时明确对框外碰撞和附加检查类型的处理。再基于真实源区域整理四向素材锚点和分层清单。有经过核验的游戏环境时继续 SMAPI 接入；缺少程序集时继续可独立完成的工作并记录具体依赖。自动续做任务已按这一顺序更新，执行时间保持原设定。
