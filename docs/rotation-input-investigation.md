...
SMAPI 右键按下／松开事件及时采样并抑制原交互，逐帧继续采样；无有效旋转选择时不接管右键。成功转向记录 Rotation preview turned，预览显示左右键用途。104/104 独立检查与匹配引用 Release 构建通过；真实事件和交互仍待实机验证。以下保留此前资格排查证据，0.1.1 的左键操作说明已被本节取代。
