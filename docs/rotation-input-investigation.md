...
## 2026-09-16 后续确认：0.1.3 显示复核通过

在解释拿起时仅隐藏旧位置绘制、取消后恢复后，用户对上一轮要求复核的门格稳定及拿起／取消显示回复“都正常了”。据此记录本批显示修正通过；本条是用户反馈，没有新增录像或逐帧测量。结合已有基本操作与保存／载入后交互反馈，本轮单栋普通空牛棚验证收尾。0.1.3 代码和包不变，不再重复这批操作，下一步回到四向美术；不据此新增动物、多实例、联机等支持声明。

## 2026-09-16：保存交互反馈与占位图闪烁

用户报告“过夜存档后—不退出能正常进出—存档正常—能正常进出，内部交互也正常”。启动日志显示 0.1.2、游戏 1.6.15、SMAPI 4.5.2，随后 Save loaded 从 False 到 True，载入后没有拿起建筑。按用户反馈记录保存／载入后交互正常；启动状态不替代逐栋位置／朝向恢复证据，不扩大成全方向、多实例或长期存档验收。含账户标识的原图不进入仓库。

另报告在“不远不近”的站位，实心薄荷色门格闪烁。代码中底色、边线、门及标签原来都是默认深度；实际世界批次 FrontToBack 及 MonoGame 的 Array.Sort 对等深度不保证提交顺序，底色可能排到门之上。0.1.3 明确各层深度；这是针对代码问题的修正，是否完全消除用户看到的现象仍需实机确认。同时在已有 Draw 补丁中隐藏有效预览所属实例的原图，消除旧图／新预览并存，取消后自然恢复。

本次只改绘制、版本与说明，原操作和保存规则不变。实际引用构建通过，发布包做内容检查；没有为颜色深度新增模拟测试或重跑未改的独立核心。换包后只复核之前会闪的位置、拿起转向及取消后的图像恢复。
