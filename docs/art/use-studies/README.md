...
新增[四步操作图](shed-operation-flow.png)（[SVG](shed-operation-flow.svg)）：拿起与预览 → 有物品时选择处理方式 → 打包核对 → 完成。它是候选流程板，不是已实现的游戏 UI；图中 OFF 仅表示该屏展示的状态，未决定默认值。

建议开关文案为“室内跟随建筑转向”，放在已进入的建筑移动流程中。开关只调整本次预览，来回切换不收走物品；未改变室内朝向就不触发打包。空房可以直接预览受支持的房间转向，有物品时显示“室内有摆放。可以保持室内原样，或先查看打包清单”，按钮为“保持室内原样”和“查看打包清单”。

打包核对页展示实际对象及状态、所需收纳箱数和放置位置；提示“打包后需要重新摆放室内物品”，按钮为“返回”和“打包并放置”。将打包确认放到最终提交建筑时，避免玩家打包后又取消搬动；此时要重新确认清单与空间仍有效，不能执行一份过期预览。失败应回到可编辑状态，成功后提示收纳位置。普通外部旋转不增加这层确认。

“保留摆放一起转”仍是另一条候选方案，当前没有做成已可用按钮；不把未实现选项伪装成能点击。暂按本次操作的选择讨论，是否记住每栋建筑的偏好、赠箱规则及准确箱数留待定案。原箱子内容和机器状态的保留责任不变。
