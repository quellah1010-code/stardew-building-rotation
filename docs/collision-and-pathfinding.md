...
公开资料可以确认数据结构与参考实现；目前不能取得用户家中安装、存档及其他 mod 修改后的最终运行时数据，也尚未核验本机牛棚的真实尺寸、门位置或通行图。
...
现有 `Footprint` 只计算矩形尺寸和内部格子的变换，**没有接入真实 `CollisionMap`、额外区域或 NPC 寻路**；`Doorway` 也只是单格边缘门的简化模型。当前 40 组独立自检不覆盖这些游戏功能，本次调查没有添加猜测的游戏补丁。
